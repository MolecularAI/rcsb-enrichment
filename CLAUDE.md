...
**Size signal: `rcsb_entry_info.deposited_polymer_monomer_count` (total residues across all protein chains in the entry), NOT per-chain sequence length.** A per-chain threshold fails for heteromeric complexes: 1JFF and 5S5V both have a tubulin alpha chain of 451 aa, so the per-chain test sees them as identical size. But 1JFF has only 2 protein chains (α+β tubulin, ~896 aa total) while 5S5V has 4 (α+β tubulin + Stathmin-4 + TTL, ~2319 aa total) — the entry-level count correctly identifies 1JFF as a structural subset. The field is fetched in `get_entry_quality` and stored in the quality dict.  
**Implementation:** three RCSB search requests using `return_type: "entry"` with `rcsb_entry_info.deposited_polymer_monomer_count` `less`/`less_or_equal`/`greater` filters. The UniProt condition selects only entries that contain at least one chain matching the accession. The sibling set is derived as the `≤upper` result minus the `<lower` fragment set (avoids a fourth request). All UniProt IDs from the same query entry share the same threshold value (the query entry's total monomer count).  
**Binder lookup tier for fragments:** fragments are treated like siblings (unconditional binder inclusion) since they cover a subset of the query complex and their cofactors are directly relevant.
